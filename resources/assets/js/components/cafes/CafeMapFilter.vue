            ...
            <div class="grid-x grid-padding-x">
                <div class="large-12 medium-12 small-12 cell">
                    ...
                    <div class="brew-methods-container">
                        <div class="filter-brew-method" v-on:click="toggleBrewMethodFilter( method.method )" :class="{ 'avtive': brewMethods.indexOf( method.method ) > -1 }"
                            v-for="method in cafeBrewMethods" :key="method.id">{{ method.method }}</div>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        computed: {
            cafeBrewMethods() {
                return this.$store.getters.getCafeBrewMethods;
            }
        },
        methods: {
            // 切换标签筛选
