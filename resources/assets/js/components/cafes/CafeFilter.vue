...
<template>
    <div id="cafe-filter">
        <div class="grid-container" v-show="show">
            <div class="grid-x grid-padding-x">
                ...
                <div class="is-roaster-container">
                    <input type="checkbox" v-model="isRoaster" /> <label>是否烘焙店？</label>
                </div>
                <!-- 冲泡方法复选组件 -->
                <div class="brew-methods-container">
                    ...
                    v-for="method in cafeBrewMethods" :key="method.id">
                        {{ method.method }}
                    </div>
                </div>
            </div>
        ...
        <!-- 显示/隐藏过滤器 -->
        <div class="grid-x grid-padding-x">
            <span class="show-filters" v-on:click="show = !show">
                {{  show ? '隐藏过滤器 ↑' :  '显示过滤器 ↓' }}
            </span>
        </div>
    </div>
...
<script>
    ...
    export default {
        ...
        // 在mountd中监听tags-edited事件，如果标签输入框的值变动，则同步过滤组件的tags字段
        mounted() {
        ...
        methods: {
            // 如果果个冲泡方法被选择，则将其推送到筛选组件的brewMethods属性，否则从brewMethods属性中将当前方法移除
        ...
        // 监听四个筛选条件的变动，任一变动则立即调用updateFilterDisplay
        watch: {
            textSearch() {
